...
function extractChannelId(url: string): { type: 'id' | 'handle' | 'custom'; value: string } | null {
  const patterns = [
    // @handle format
...
channelsRouter.post('/:id/refresh-videos', async (c) => {
  ...
  try {
    ...
    if (!playlistData.items || playlistData.items.length === 0) {
      return c.json({ message: 'No new videos found', updated: 0, total: existingVideoIds.size });
    }
    ...
    logger.debug(
      {
        ...
        first3: playlistData.items
          .slice(0, 3)
          .map((item: { snippet: { title: string }; contentDetails: { videoId: string } }) => ({
    ...
    if (!detailsResponse.ok) {
      logger.error({ errorText: await detailsResponse.text() }, 'YouTube Videos API error');
    ...
    const videosToUpdate = detailsData.items.filter((ytVideo: YouTubeVideoDetails) =>
      existingVideoIds.has(ytVideo.id)
    );

    logger.info(
      { newVideosCount: newVideos.length, videosToUpdateCount: videosToUpdate.length },
      'Video sync status'
    );
    ...
    if (videosToUpdate.length > 0) {
      ...
      logger.info({ count: videosToUpdate.length }, 'Updated statistics for existing videos');
    }
    ...
    const videosToInsert = newVideos.map((ytVideo: YouTubeVideoDetails) => ({
      ...
      thumbnail_url:
        ytVideo.snippet.thumbnails?.high?.url || ytVideo.snippet.thumbnails?.default?.url,
    ...
    await supabase.from('channels').update({ video_count: newTotal }).eq('id', id);
