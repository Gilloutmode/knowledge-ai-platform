...
webhooksRouter.post("/video-analyzed", async (c) => {
  try {
    ...
    if (!supabase) {
      return c.json({
        success: true,
        message: "Demo mode - video analysis received",
      });
    }

    // Insert analyses
    if (analyses.length > 0) {
      const analysesToInsert = analyses.map(
        (analysis: WebhookAnalysisPayload) => ({
          video_id: videoId,
          type: analysis.type,
...
webhooksRouter.post("/content-generated", async (c) => {
  try {
    ...
    if (!supabase) {
      return c.json({
        success: true,
        message: "Demo mode - content generation received",
      });
    }
...
webhooksRouter.post("/new-video-detected", async (c) => {
  try {
    ...
    logger.info(
      { channelId, videosCount: videos.length },
      "New videos detected",
    );
    ...
    if (!supabase) {
      return c.json({
        success: true,
        message: "Demo mode - new videos detected",
      });
    }
