...
async function request<T>(endpoint: string, options: RequestInit = {}, retryCount = 0): Promise<T> {
  try {
    ...
    if (response.status === 429) {
      ...
      throw new ApiError(429, 'Too many requests. Please try again later.', false, retryAfter);
    }
...
export const channelsApi = {
  preview: async (youtubeUrl: string): Promise<ChannelPreview> => {
    const data = await request<{ channel: ChannelPreview }>('/channels/preview', {
      method: 'POST',
  ...
  add: async (youtubeUrl: string): Promise<Channel> => {
    const data = await request<{ channel: Channel; message: string }>('/channels', {
      method: 'POST',
      body: JSON.stringify({ youtube_url: youtubeUrl }),
    });
    return data.channel;
  ...
  refreshVideos: async (
    ...
    const data = await request<{ message: string; updated: number; total: number }>(
      `/channels/${id}/refresh-videos`,
      { method: 'POST' }
    );
    return data;
  },
...
export const videosApi = {
  listByChannel: async (channelId: string, params?: PaginationParams): Promise<VideosResponse> => {
  ...
  analyze: async (id: string, types: string[], language: 'fr' | 'en' = 'fr'): Promise<void> => {
...
export const analysesApi = {
  ...
  listByVideo: async (videoId: string): Promise<Analysis[]> => {
    const data = await request<{ analyses: Analysis[] }>(`/analyses?videoId=${videoId}`);
    return data.analyses;
  },

  get: async (id: string): Promise<AnalysisWithVideo> => {
    const data = await request<{ analysis: AnalysisWithVideo }>(`/analyses/${id}`);
...
export const searchApi = {
  global: async (query: string): Promise<SearchResults> => {
    ...
    const data = await request<SearchResults>(`/search?q=${encodeURIComponent(query)}`);
    return data;
  },
};
